...
extern void    spi_multi_timing_set(void);
extern uint8_t spi_multi_cmd_read(uint8_t command);
extern void    spi_multi_cmd_write(uint8_t command, uint8_t size, uint32_t data);
extern void    spi_multi_page_program(uint32_t addr, uintptr_t buff);
extern void    spi_multi_erase_sector(uint32_t addr);
